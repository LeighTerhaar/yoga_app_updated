class FavoritesController < ApplicationController
  def index
    ...
    @favorites = @q.result(:distinct => true).includes(:user, :yogaflow).page(params[:page]).per(10)
  ...
  def create_row_from_yogaflow
    ...
    if @favorite.valid?
      ...
      redirect_to("/yogaflows/#{@favorite.yogaflow_id}", notice: "Favorite created successfully.")
    else
      render("favorite_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_yogaflow
    ...
    redirect_to("/yogaflows/#{@favorite.yogaflow_id}", notice: "Favorite deleted successfully.")
  end
